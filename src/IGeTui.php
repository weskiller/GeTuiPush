...
use Weskiller\GeTuiPush\Utils\LangUtils;
...
Class IGeTui
{
    ...
    public function __construct($domainUrl, $appkey, $masterSecret, $ssl = NULL)
    {
        ...
        if ($ssl === NULL && $domainUrl !== NULL && stripos($domainUrl, "https:") === 0)
        {
            $ssl = true;
        }
    ...
    private function initOSDomain($hosts)
    {
        ...
        {
            $hosts = IGeTui::$appkeyUrlList[$this->appkey] ?? null;
            if($hosts === null || count($hosts) === 0)
            {
    ...
    public function getOSPushDomainUrlList($domainUrlList,$appkey)
    {
        ...
        if($urlList === null || count($urlList) <= 0)
        {
            ...
            throw new RuntimeException("Can not get hosts from ".$h."|error:".$ex);
        }
        return $urlList;
    }
    ...
    public  function connect()
    {
        ...
        throw new RuntimeException("appKey Or masterSecret is Auth Failed");
    }
     ...
     * @return array $requestId {result:successed_offline,taskId:xxx}  || {result:successed_online,taskId:xxx} || {result:error}
     **/
    ...
    private function blackCidList($appId,$cidList,$optType){
        ...
        if($limit < count($cidList)){
            throw new RuntimeException("cid size:".count($cidList)." beyond the limit:".$limit);
        }
     ...
     *  批量推送信息
     * @param  String $contentId
     * @param  array $targetList <IGtTarget> targetList
    ...
    private function getListAppContentId($message, $taskGroupName = null)
    {
        ...
        if (!is_null($taskGroupName) && trim($taskGroupName) != ""){
            if(strlen($taskGroupName) > 40){
                throw new RuntimeException("TaskGroupName is OverLimit 40");
            }
        ...
        throw new RuntimeException("host:[".$this->host."]" . "获取contentId失败:" . $rep);
    }
    ...
    public function getUserCountByTags($appId, $tagList) {
        ...
        if(count($tagList) > $limit) {
            throw new RuntimeException("tagList size:".count($tagList)." beyond the limit:".$limit);
        }
        return $this->httpPostJSON($this->host, $params);
    }
    ...
    public function queryAppPushDataByDate($appId, $date){
        if(!LangUtils::validateDate($date)){
            throw new RuntimeException("DateError|".$date);
        }
        $params = array();
