<?php
namespace Weskiller\GeTuiPush\IGeTui\Template;

use RuntimeException;
use Weskiller\GeTuiPush\IGeTui\ActionChain;
use Weskiller\GeTuiPush\IGeTui\ActionChainType;
use Weskiller\GeTuiPush\IGeTui\AppStartUp;
use Weskiller\GeTuiPush\IGeTui\NotifyInfo;
...
class IGtTransmissionTemplate extends IGtBaseTemplate {
	...
	public function  getActionChain() {
		...
		$actionChain2->set_autostart($this->transmissionType === '1');
    ...
    function set3rdNotifyInfo($notify) {
        if ($notify->get_title() === null || $notify -> get_content() === null) {
            throw new RuntimeException("notify title or content cannot be null");
        }
        ...
        if($notify->get_type() > -1){
            ...
            if($notify -> get_intent() !== null){
                if(strlen($notify -> get_intent()) > GTConfig::getNotifyIntentLimit()){
                    throw new RuntimeException("intent size overlimit " . GTConfig::getNotifyIntentLimit());
                }
                //不符合intent的格式要求
                if(!preg_match(self::pattern,$notify -> get_intent())){
                    throw new RuntimeException("intent format err,should start with \"intent:#Intent;\",end \"with ;end\"  ");
                }
